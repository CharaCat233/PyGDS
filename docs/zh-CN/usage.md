...
列表支持长度可变的切片赋值与切片删除（含扩展切片，步长非 1 时要求等长）：
...
> **⚠️ 已知差异**：`yield` 恢复采用语句重执行续延，`yield` 所在语句中位于挂起点之前的子表达式按「节点 + 出现次序」记忆（如 `f(a(), (yield 1))` 中的 `a()` 只执行一次）；同一语句内多个「结构等值」的调用仍可能互相认错帧，彻底解决需要表达式级续延
